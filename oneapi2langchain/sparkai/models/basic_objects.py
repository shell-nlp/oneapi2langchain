...
class BaseObject:
    ...
    def __str__(self):
        return f"<sparkai.{self.__class__.__name__}>"
...
class JsonObject(BaseObject, metaclass=ABCMeta):
    ...
    def validate_json(self) -> None:
        """
        Raises:
          SparkAIObjectFormationError if the object was not valid
        """
    ...
    def __repr__(self):
        ...
        if dict_value:  # skipcq: PYL-R1705
            return f"<sparkai.{self.__class__.__name__}: {dict_value}>"
        else:
            return self.__str__()
...
class JsonValidator:
    def __init__(self, message: str):
        ...
        Args:
            message: Message to be attached to the thrown SparkAIObjectFormationError
        """
    ...
    def __call__(self, func: Callable) -> Callable[..., None]:
        ...
        def wrapped_f(*args, **kwargs):
            if not func(*args, **kwargs):
                raise SparkAIObjectFormationError(self.message)
